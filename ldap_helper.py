...
class LdapHelper:
    ...
    def search(self, user: str) -> Tuple[Optional[dict], ldap3.Connection]:
        ...
        for user_group, current_filter in [
            (self.ADMIN_GROUP, self.admin_filter),
            (self.USER_GROUP, self.user_filter),
        ]:
            con.search(
                self.base_dn,
                current_filter.format(**filter_args),
            ...
            if len(result) > 0:
                if len(result) > 1:
                    LOG.debug(
                        "Found more than one entry for filter %s"
                        % (current_filter.format(**filter_args))
                    )
                    LOG.info(
                        ("Found more than one entry for filter args %s" % (filter_args))
                    )
                    return None, con
...
@click.group()
@click.option("--host", "-h", type=str, required=True, help="LDAP server to connect")
@click.option(
    "--bind-dn",
    type=str,
    required=True,
...
@click.option(
    "--bind-dn-password", type=str, required=True, help="Password for the bind dn user"
)
@click.option(
    "--base-dn", type=str, required=True, help="Base DN to use when looking up users"
)
@click.option(
    "--admin-filter",
    type=str,
...
@click.option("--ssl", is_flag=True, default=False, help="Use SSL connection")
@click.option(
    "--log-level",
...
def group(
    ...
    helper = LdapHelper(
        host, bind_dn, bind_dn_password, base_dn, admin_filter, user_filter, ssl
    )
...
@group.command()
@click.option("--user", "-u", type=str, required=True, help="User to look up")
@click.pass_context
def search(ctx, user):
...
@group.command()
@click.option("--unix-socket", required=False, help="Run on a unix socket")
